...
function getEventInfo(activity, difficulty) {
    let icon = '';
    let type = 'workout'; // Default to Dark Green
    
    const act = (activity || '').toLowerCase();
    const diff = (difficulty || '').toLowerCase();
    
    if (act.includes('treadmill')) {
        // TREADMILL LOGIC BASED ON DIFFICULTY
        if (diff.includes('easy')) {
            icon = '🏃‍♂️'; 
            type = 'workout-easy'; // Light Green
        } else {
            // Default or 'Difficult'
            icon = '🔥'; 
            type = 'workout';      // Dark Green
        }
...
async function loadData() {
    try {
        ...
        // Auto-detect columns (Now looking for 'Difficulty')
        const headers = rows[0].map(h => h.toLowerCase());
        const idxDate = headers.findIndex(h => h.includes('date')); 
        const idxAct = headers.findIndex(h => h.includes('activity'));
        const idxDiff = headers.findIndex(h => h.includes('difficulty')); // New Column
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            dateList.forEach(dateStr => {
                events.push({
                    ...
                    difficulty: idxDiff > -1 ? row[idxDiff] : '', // Capture Difficulty
                    speed: row[idxSpeed] || '',
                    duration: row[idxDur] || '',
                    notes: row[idxNote] || ''
                });
        ...
        console.error(e);
        calendar.innerHTML = `<div style="color:red; text-align:center;">Error loading data.</div>`;
    }
}
...
function expandDateRange(rawStr) {
    try {
        ...
        if (cleanStr.includes("-")) {
            ...
            if (!isNaN(d)) resultDates.push(formatDate(d));
        }
        return resultDates;
    } catch (err) { return []; }
}
...
function renderCalendar() {
    ...
    for(let i = 1; i <= daysInMonth; i++) {
        ...
        if (eventData) {
            // PASS BOTH ACTIVITY AND DIFFICULTY
            ...
            if (type.includes('workout')) {
                // Show Details for Workout (Easy or Hard)
                detailsHTML = `
            ...
            daySquare.innerHTML = `
                ...
                <div class="day-content">${detailsHTML}</div>
            `;
